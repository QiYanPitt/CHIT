...
def write_empty_result(outfile, snpinfo):
    """Write all zeros in the event that the test failed"""
    outfile.write("\t".join([snpinfo[0][0], snpinfo[0][1], "0", "0", "0", "NA", 
        str(int(tot_as_counts)), str(int(ref_as_counts)), str(int(alt_as_counts)), str(int(all_counts))]) + '\n')
    ...
    try:
        ...
        for start in starting_gene:
            ...
            res = minimize(ll.ll_one, starts, args=(disease_stat, test_snps, True, options.is_as_only, bnb_sigmas, as_sigmas,
                           options.read_error_rate, [], cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER1)
            new_par = res.x
        ...
        for cov in range(num_covs):
            res = minimize(ll.ll_cov, [np.float64(0)], args=(starting_par, disease_stat, test_snps, True, options.is_as_only, bnb_sigmas, as_sigmas,
                                 options.read_error_rate, cov_coefs, cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER1)
            new_coef = res.x
            cov_coefs = np.concatenate([cov_coefs, new_coef])
        res = minimize(ll.ll_one, starting_par, args=(disease_stat, test_snps, options.is_bnb_only, options.is_as_only, bnb_sigmas,
                             as_sigmas, options.read_error_rate, cov_coefs, cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER1)
        best1par = res.x
        ...
        res = minimize(ll.ll_two, start, args=(disease_stat, test_snps, options.is_bnb_only, options.is_as_only, bnb_sigmas, as_sigmas,
                       options.read_error_rate, cov_coefs, cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER1)
        best2par = res.x
        ...
        if pval >= 1e-2:
            ...
            for start in starting_gene:
                starts = [np.float64(options.initial), np.float64(options.initial), np.float64(0), np.float64(start)]
                res = minimize(ll.ll_one, starts, args=(disease_stat, test_snps, True, options.is_as_only, bnb_sigmas, as_sigmas, 
                               options.read_error_rate, [], cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER2)
                new_par = res.x
                new_loglike = ll.ll_one(new_par, disease_stat, test_snps, options.is_bnb_only, options.is_as_only, bnb_sigmas,
                                                as_sigmas, options.read_error_rate, [], cov_matrix)
                if new_loglike < maxlike:
            ...
            for cov in range(num_covs):
                res = minimize(ll.ll_cov, [np.float64(0)], args=(starting_par, disease_stat, test_snps, True, options.is_as_only, bnb_sigmas, as_sigmas,
                                     options.read_error_rate, cov_coefs, cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER2)
                new_coef = res.x
                cov_coefs = np.concatenate([cov_coefs, new_coef])
            res = minimize(ll.ll_one, starting_par, args=(disease_stat, test_snps, options.is_bnb_only, options.is_as_only, bnb_sigmas,
                                 as_sigmas, options.read_error_rate, cov_coefs, cov_matrix), options={"maxiter" : 50000, "disp" : options.verbose}, method=OPTIMIZER2)
            best1par = res.x
            loglike1par = ll.ll_one(best1par, disease_stat, test_snps, options.is_bnb_only, options.is_as_only, bnb_sigmas,
                                             as_sigmas, options.read_error_rate, cov_coefs, cov_matrix)
            start = [best1par[0], best1par[1], best1par[2], best1par[2], best1par[3]]
            ...
            loglike2par = ll.ll_two(best2par, disease_stat, test_snps, options.is_bnb_only, options.is_as_only, bnb_sigmas,
                                            as_sigmas, options.read_error_rate, cov_coefs, cov_matrix)
            chisq = 2 * (loglike1par - loglike2par)
            pval2 = (1-scipy.stats.chi2.cdf(chisq,1))
            #print "p-value2 is %r" % pval2
            if pval2 >= 1e-2:
                write_results.write_results(outfile, snpinfo, loglike1par, loglike2par, best2par, tot_as_counts, ref_as_counts, alt_as_counts, all_counts)
            elif pval2 == 0:
                write_empty_result(outfile, snpinfo)
            elif pval == 0 and pval2 < 1e-2 and pval2 > 0:
                write_empty_result(outfile, snpinfo)
            elif pval < 1e-2 and pval > 0 and pval2 < 1e-2 and pval2 > 0:
                ratio = pval/pval2
